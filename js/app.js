...
const FORMATS={
  ...
  alkalinity:{decimals:2,unit:" dKH",min:8.0,max:8.4,watch:.4},
...
const classify=(value,rule)=>{
  ...
  if(
    value>=rule.min-rule.watch &&
    value<=rule.max+rule.watch
  ){
    return "watch";
  }
...
const updateSystems=()=>{
  ...
  const tridentIds=[
    ENTITY_IDS.alkalinity,
    ENTITY_IDS.calcium,
    ENTITY_IDS.magnesium
  ];

  const missing=tridentIds.some(
    id=>numericState(id)===null
  );

  const oldest=Math.max(
    ...tridentIds
      .map(freshnessMinutes)
      .filter(v=>v!==null),
    0
  );
  ...
  const ages=Object.keys(FORMATS)
    .map(key=>freshnessMinutes(ENTITY_IDS[key]))
    .filter(v=>v!==null);

  const newest=ages.length
    ?Math.min(...ages)
    :null;
  ...
  setSystem(
    ...
    newest===null
      ?"No updates"
      :newest>15
        ?`${Math.round(newest)} min old`
        :"Fresh"
  );
};

const updateBrain=()=>{
  const checks=Object.entries(FORMATS).map(
    ([key,rule])=>{
  ...
  const watches=checks.filter(
    x=>x.state==="watch"
  );

  const stable=checks.filter(
    x=>x.state==="good"
  );
  ...
  if(alerts.length){
    ...
    summary.textContent=
      `${alerts.map(x=>x.key).join(", ")} ` +
      `${alerts.length===1?"is":"are"} outside the preferred range. ` +
      `Verify the readings before making changes.`;

    priority.textContent=
      `Review ${alerts.map(x=>x.key).join(", ")}`;

  }else if(watches.length){

    headline.textContent=
      "A few readings deserve attention";

    summary.textContent=
      `${watches.map(x=>x.key).join(", ")} ` +
      `${watches.length===1?"is":"are"} near the edge of the target range. ` +
      `Keep conditions steady and confirm the trend first.`;

    priority.textContent=
      `Monitor ${watches.map(x=>x.key).join(", ")}`;

  }else{

    headline.textContent=
      "Dreams of Tahiti is stable";

    summary.textContent=
      "All available chemistry readings are within their preferred operating ranges. " +
      "No immediate chemistry adjustment is recommended.";
...
const updateDashboard=(states)=>{
  ...
  document.getElementById(
    ...
    textState(
      ENTITY_IDS.mission,
      "Waiting for Reef Doctor..."
    );

  Object.entries(FORMATS).forEach(
    ([key,format])=>{

      const value=numericState(
        ENTITY_IDS[key]
      );
...
window.parent.postMessage(
  {
    type:"reef-mission-control-v2-ready"
  },
